 ...
 * its claims. order-service shares the same {@code app.jwt.secret} as auth-service
 * but only ever *verifies* tokens. The signature is always recomputed with
 * HMAC-SHA256, so the alg header is irrelevant (no alg-confusion).
 */
@Service
@Slf4j
