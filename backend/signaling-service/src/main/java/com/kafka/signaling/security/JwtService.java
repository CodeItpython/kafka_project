...
/**
 * Verifies the HS256 JWT minted by auth-service and reconstructs the caller from
 * its claims. signaling-service shares the same {@code app.jwt.secret} as
 * auth-service but only ever *verifies* tokens. The signature is always recomputed
 * with HMAC-SHA256, so the alg header is irrelevant (no alg-confusion). This mirrors
 * chat-service's verify-only JwtService so both services trust the same tokens.
 */
@Service
@Slf4j
