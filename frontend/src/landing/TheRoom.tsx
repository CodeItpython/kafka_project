...
// 마우스를 올리거나 포커스하면 짧은 설명이 나타난다 (팝업 없음). 터치 기기에서는 항상 표시(CSS).
const ITEMS = [
...
export default function TheRoom({ container, reduce }: { container: RefObject<HTMLDivElement | null>; reduce: boolean }) {
  return (
    <section className="uc-sec uc-room" id="room" aria-labelledby="room-title">
      <LazyVideo container={container} src={MEDIA.room} className="uc-room-video" reduce={reduce} />
      ...
      <div className="uc-grid uc-room-grid">
        ...
        <ul className="uc-col-body uc-room-list">
          {ITEMS.map((it, i) => (
            <li key={it.k}>
              <div className="uc-room-item" tabIndex={0} aria-describedby={`room-desc-${i}`}>
                <span>{it.k}</span>
                <small id={`room-desc-${i}`}>{it.d}</small>
              </div>
            </li>
          ))}
        </ul>
