...
- 영상이 없으면 `Hero`가 **실시간 three.js 3D 씬**(`src/landing/scene/HeroScene.tsx`)을 지연 로드해 같은 3막(키보드 조립·타건 → 빛의 스트림 → 기기 점등)을 스크롤 진행도로 스크러빙한다.
  reduced-motion 이거나 WebGL 이 실패하면 힉스필드 **Nano Banana Pro(2K)** 스틸 3장 크로스페이드로 내려간다: `hero.jpg`/`hero-still-a.jpg`(키캡 조립), `hero-still-b.jpg`(스트림 통로), `hero-last.jpg`/`hero-still-c.jpg`(완성 기기).
  우선순위: 영상(`hero.mp4`) > 3D 씬 > 스틸.
